(function () {
  ...
  async function initLoginPage() {
    ...
    form.addEventListener('submit', async function (e) {
      ...
      try {
        ...
        text(msg, 'Login berhasil. Redirect ke simulasi...');
        window.location.href = '/simulasi';
      } catch (err) {
  ...
  async function initSimulasiPage() {
    ...
    const levels = [...new Set(subjects.map((s) => s.education_level))];
    levels.forEach((lvl) => {
    ...
    levelSelect.addEventListener('change', function () {
      ...
      if (!level) return;
      const types = [...new Set(subjects.filter((s) => s.education_level === level).map((s) => s.subject_type))];
    ...
    typeSelect.addEventListener('change', function () {
      ...
      subjects
        .filter((s) => s.education_level === level && s.subject_type === type)
        .forEach((s) => {
          const o = document.createElement('option');
          o.value = String(s.id);
  ...
  async function initAttemptPage() {
    ...
    function collectPayload(q) {
      ...
      const selected = Array.from(selectedNodes).map((n) => String(n.value || '').trim()).filter(Boolean);
      if (qType === 'pg_tunggal') {
    ...
    try {
      ...
      out.items.forEach(function (it, idx) {
        ...
        const breakdown = Array.isArray(it.breakdown) && it.breakdown.length
          ? '<br><small>' + it.breakdown.map((b) => escapeHtml(b.id + ':' + (b.correct ? 'ok' : 'x'))).join(', ') + '</small>'
          : '';
        tr.innerHTML = [
          '<td>' + (idx + 1) + '</td>',
