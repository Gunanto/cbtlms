(function () {
  ...
  async function initSimulasiPage() {
    ...
    levelSelect.addEventListener("change", function () {
      ...
      const types = [
        ...new Set(
          subjects
            .filter((s) => s.education_level === level)
            .map((s) => s.subject_type),
        ),
      ];
  ...
  async function initAttemptPage() {
    ...
    function renderOptions(q) {
      let payload = {};
      try {
        payload = JSON.parse(q.answer_payload || "{}");
      } catch (_) {}
      ...
      if (qType === "benar_salah_pernyataan") {
        ...
        const rows = statements.map((s, idx) => {
          ...
          return [
            ...
            '<label><input type="radio" name="bs_' +
              escapeHtml(id) +
              '" value="true" ' +
              yesChecked +
              "> Benar</label>",
      ...
      out.items.forEach(function (it, idx) {
        ...
        const breakdown =
          Array.isArray(it.breakdown) && it.breakdown.length
            ? "<br><small>" +
              it.breakdown
                .map((b) => escapeHtml(b.id + ":" + (b.correct ? "ok" : "x")))
                .join(", ") +
              "</small>"
            : "";
        tr.innerHTML = [
